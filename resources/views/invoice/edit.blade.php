        ...
        <form method="POST" action="{{route('invoices.update' , ['invoice' => $invoice->id])}}">
            ...
            <div class="mb-3">
              ...
              <input type="text" class="form-control" id="id" aria-describedby="codigoHelp" name="id"
                    disabled="disabled" value="{{ $invoice->id }}">
              <div id="codigoHelp" class="form-text">Factura Id.</div>
            </div>
            <div class="mb-3">
              <label for="name" class="form-label">Factura numero</label>
              <input type="text" required class="form-control" id="name" placeholder="Factura numero" 
                    name="number" value="{{ $invoice->number }}">             
            </div>
            ...
            <select class="form-select" id="customer" name="code" required>
                ...
                @foreach ( $customers as $customer )
                    @if ($customer->id == $invoice->id)                      
                        <option selected value="{{ $customer->id }}">{{ $customer->last_name }}</option>
                    @else
              ...
              <div class="mb-3">
                ...
                <input type="text" required class="form-control" id="name" placeholder="date" 
              ...
              <select class="form-select" id="pay_mode" name="code" required>
                  ...
                  @foreach ( $pay_mode as $pay_modes )
                      @if ($pay_modes->id == $invoice->id)                      
                          <option selected value="{{ $pay_modes->id }}">{{ $pay_modes->name }}</option>
                      @else
